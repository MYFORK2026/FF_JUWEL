...
module.exports.run = async function ({ api, event, permssion }) {
  ...
  if (permssion < 2) {
    return api.sendMessage(
      "❌ এই কমান্ড শুধু এডমিন ব্যবহার করতে পারবে!",
      event.threadID,
      event.messageID
    );
  }
  ...
  // ----------- তোমার মিডিয়া লিংকগুলা (SFW) ----------
  // চাইলে এখানে যেকোনো লিংক বসাতে পারো
  const mediaLinks = [
  ...
  // ----------- ডুপ্লিকেট ভিডিও না পাঠানোর সিস্টেম ----------
  let randomLink;
  const lastSent = this.lastSentMedia || null;

  do {
    randomLink = mediaLinks[Math.floor(Math.random() * mediaLinks.length)];
  } while (mediaLinks.length > 1 && randomLink === lastSent);

  this.lastSentMedia = randomLink;

  // ----------- ফাইল পাথ ----------
  const filePath = __dirname + "/cache/admin_media.mp4";
  ...
  request(encodeURI(randomLink))
    ...
    .on("error", (err) => {
      ...
      api.sendMessage("❌ মিডিয়া ডাউনলোড করতে সমস্যা হয়েছে!", event.threadID);
    });
